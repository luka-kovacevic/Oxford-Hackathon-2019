...
class RoutePostcodeConverter :
    ...
    def dictionaryPostToIndex(self) : 
    ...
    def incidenceToMatrix(self, dictPostcodes) :
        postcodes = []
        for currentRouteValue in self.routeIndices :
            currentPostcode = dictPostcodes[currentRouteValue]
            postcodes.append(currentPostcode)
        return postcodes
    
    #uses above two matrices to complete the conversion
    def getPostcodeRoute(self):
        dictionary = self.dictionaryPostToIndex()
        routePostcodes = self.incidenceToMatrix(dictionary)
